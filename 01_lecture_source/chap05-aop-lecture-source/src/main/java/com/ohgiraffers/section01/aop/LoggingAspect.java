...
public class LoggingAspect {
    ...
    public void logBefore(JoinPoint joinPoint) {
        ...
        System.out.println("Before joinPoint.getTarget() : " + joinPoint.getTarget()); // 클래스 객체 정보
        System.out.println("Before joinPoint.getSignature() : " + joinPoint.getSignature()); // return이 실행 되었다.

        if (joinPoint.getArgs().length > 0) {

            System.out.println("Before joinPoint.getArg() : " + joinPoint.getArgs()[0]);
        }
    }

    /* After 어드바이스는 포인트 컷을 동일한 클래스 내에서 사용하는 것이면 클래스 명은 생략 가능하다.
    ...
    * 또한 joinPoint는 반드시 첫번째 매개변수로 선언해야한다. */
    ...
    * 심지어 원본 조인 포인트를 언제 실행 할지, 실행 자체를 안할지, 계속 실행할지 여부까지도 제어한다.
    * AroundAdvice는 조인 포인트 매개변수는 ProceedingJoinPoint로 고정되어있다.
    * JoinPoint의 하위 인터페이스로 원본 조인포인트의 진행 시점을 제어할 수 있다.
    * 조인 포인트의 진행하는 호출을 잊는 경우가 자주 발생하기 때문에 주의 해야하며
